...
For each (checkpoint, K) combination, measures:
  • Clean accuracy   (EOT purification)
  • PGD-20 robust accuracy  (BPDA straight-through)
  • Purification time  (ms/image, wall-clock)
  • UNet calls per sample  (actual count)
  • Speedup vs. K=1 baseline

Output: results/cifar10/deepcache_results.csv  (append — never overwrites)
        results/cifar10/deepcache_results.json

Usage:
  # Full sweep — all checkpoints, K in [1,2,3,5], 512 images, PGD-20
  PYTORCH_CUDA_ALLOC_CONF=expandable_segments:True \\
  python3 cifar10_deepcache_sweep.py --device cuda:0

  # Quick clean-only sweep (no PGD — much faster)
  python3 cifar10_deepcache_sweep.py --only_clean --eval_images 256 --device cuda:0

  # Custom checkpoints and K values
  python3 cifar10_deepcache_sweep.py \\
      --checkpoints baseline 10pct 25pct \\
      --k_values 1 2 5 \\
      --device cuda:1
"""
